...
const CourseDetailPage = () => {
  ...
  const fetchCourseDetails = useCallback(async () => {
    try {
      ...
      const isAuthor = user && courseData.author && courseData.author._id === user.id;
    ...
    <div className="pt-20 p-8 min-h-[calc(100vh-80px)] bg-bg-primary text-text-primary">
      <div className="bg-bg-secondary p-8 rounded-lg shadow-lg border border-border-color">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <p><span className="font-semibold">Autor:</span> {course.author ? `${course.author.name} ${course.author.last_name}` : 'Desconocido'}</p>
          <p><span className="font-semibold">Materia:</span> {course.subject ? course.subject.name : 'Desconocida'}</p>
          <p><span className="font-semibold">Nivel:</span> {course.nivel}</p>
        </div>
        ...
        <div className="my-6 p-4 border border-border-color rounded-lg text-center">
          {isAuthenticated ? (
            isEnrolled ? (
              ...
              >
                {enrollmentLoading ? 'Inscribiendo...' : 'Inscribirse al Curso'}
              </CustomButton>
            )
          ) : (
            <p className="text-text-secondary">Inicia sesión para inscribirte en este curso.</p>
          )}
        </div>

        <h2 className="text-2xl font-bold text-text-primary mb-4">Lecciones</h2>
        {lessons.length === 0 ? (
          <p className="text-text-secondary">No hay lecciones disponibles para este curso.</p>
        ) : (
          <ul className="space-y-4">
            {lessons.map((lesson) => (
              <li key={lesson._id} className="bg-bg-primary p-4 rounded-lg shadow-sm border border-border-color flex justify-between items-center">
                <div>
                  <h3 className="text-xl font-semibold text-text-primary">{lesson.order}. {lesson.title}</h3>
