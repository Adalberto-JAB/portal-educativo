    ...
    <div className="bg-bg-secondary p-4 rounded-lg shadow-md border border-border-color flex flex-col h-full">
      <img
        ...
        onError={(e) => {
          ...
          e.target.src =
            "https://placehold.co/400x200/cccccc/333333?text=Error";
        }}
      />
      <div className="flex flex-col flex-grow">
        <h2 className="text-lg sm:text-xl text-center font-semibold text-text-primary mb-4">
          {course.title}
        </h2>
        ...
        <div className="flex flex-col text-sm text-text-secondary mb-6 px-4 sm:px-6">
          ...
          <span>
            Materia: {course.subject ? course.subject.name : "Desconocida"}
          </span>
        </div>
        <CustomButton
          variant="primary"
...
const CoursesPage = () => {
  ...
  const filteredAndSortedCourses = useMemo(() => {
    ...
    if (!isAuthenticated || user.role === "guest") {
      currentCourses = currentCourses.filter(
        (course) => course.isGuestViewable && course.isPublished
      );
    ...
    if (selectedSubject) {
      currentCourses = currentCourses.filter(
        (course) => course.subject?._id === selectedSubject
      );
    }
  ...
  return (
    <div className="mt-28 p-4 sm:p-6 md:p-8 min-h-screen bg-bg-primary text-text-primary">
