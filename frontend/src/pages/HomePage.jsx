...
const HomePage = () => {
  ...
  const fetchHomePageData = useCallback(async () => {
    ...
    setDateRange(
      `del ${today.getDate()} de ${today.toLocaleString("es-ES", {
        month: "long",
      })} al ${endDate.getDate()} de ${endDate.toLocaleString("es-ES", {
        month: "long",
      })}`
    );
    ...
    try {
      const fetchedHolidays = await historyAndHolidaysService.getHolidays(
        currentYear
      );
    ...
    try {
      const fetchedFacts =
        await historyAndHolidaysService.getHistoricalFactsForWeek(
          currentMonth,
          currentDay
        );
  ...
  return (
    <div className="mt-32 flex flex-col items-center justify-center min-h-[calc(100vh-80px)] p-4 bg-bg-primary text-text-primary">
      <div className="max-w-[1024px] mx-auto">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold mb-4 md:mb-12 text-center">
          ¡Bienvenido al Portal Educativo!
        </h1>
        <p className="text-base sm:text-lg md:text-xl text-center mb-8 md:mb-12">
          Tu plataforma integral para el aprendizaje y desarrollo.
        </p>
        <div className="flex flex-col sm:flex-row gap-4 mb-8 sm:justify-center md:mb-12">
          <CustomButton variant="primary" onClick={() => navigate("/courses")}>
            Explorar Cursos
          </CustomButton>
          <CustomButton
            variant="secondary"
        ...
        <div className="w-full max-w-4xl space-y-8">
                  ...
                  <tbody>
                    {holidays.map((holiday, index) => (
                      <tr key={index} className="border-b border-border-color">
                        <td className="p-3 md:p-4 align-top font-medium whitespace-nowrap">
                          {new Date(holiday.fecha).toLocaleDateString("es-AR", {
                            day: "numeric",
                            month: "long",
                          })}
                        </td>
          ...
          <div className="bg-bg-secondary p-4 md:p-6 rounded-lg shadow-md border border-border-color">
            <h2 className="text-xl md:text-2xl font-bold mb-4">
              Hechos Históricos de la Semana {dateRange}
            </h2>
            {loadingFacts ? (
              <Loader />
            ) : errorFacts ? (
                ...
                <table className="w-full text-left">
                  ...
                  <tbody>
                    {historicalFacts.map((dayFacts, index) => (
                      <tr key={index} className="border-b border-border-color">
                        <td className="p-3 md:p-4 align-top font-semibold whitespace-nowrap">
                          {dayFacts.date}
                        </td>
