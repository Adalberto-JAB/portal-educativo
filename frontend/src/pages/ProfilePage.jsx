      ...
      <div className="bg-bg-secondary p-8 rounded-lg shadow-lg w-full max-w-2xl mx-auto border border-border-color">
        <div>
          ...
          <div className="border-t border-border-color pt-4">
            <p className="text-lg mb-2"><span className="font-semibold">Rol:</span> {profile.role}</p>
            <p className="text-lg mb-2"><span className="font-semibold">Dirección:</span> {profile.address || 'No especificada'}</p>
            <p className="text-lg mb-2"><span className="font-semibold">Teléfono:</span> {profile.phoneNumber || 'No especificado'}</p>
          </div>
          <div className="flex justify-end mt-6 gap-4">
            <CustomButton variant="secondary" type="button" onClick={() => navigate('/profile/change-password')}>
              Cambiar Contraseña
            </CustomButton>
            <CustomButton variant="primary" type="button" onClick={() => navigate('/profile/edit')}>
              Editar Perfil
            </CustomButton>
          </div>
        </div>
