...
const AsignaturaManagementPage = () => {
  ...
  const fetchAsignaturas = async () => {
    try {
      ...
      setAsignaturas(sortedData);
      setError(null);
    } catch (err) {
      console.error("Error fetching asignaturas:", err);
      setError(
        "No se pudieron cargar las asignaturas. Por favor, inténtalo de nuevo más tarde."
      );
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire(
            'Error',
            err.response?.data?.message || "Error al eliminar la asignatura.",
            'error'
          )
        } finally {
          setLoading(false);
  ...
  const currentItems = asignaturas.slice(indexOfFirstItem, indexOfLastItem);
    ...
    <div className="mt-28 p-8 min-h-[calc(100vh-80px)] bg-bg-primary text-text-primary">
      <div className="max-w-[1024px] mx-auto">
        ...
        <div className="flex justify-end-safe mb-8">
          <CustomButton
            variant="primary"
            onClick={() => navigate("/admin/asignaturas/create")}
          >
                    ...
                    <tr key={asignatura._id}>
                      ...
                      <td className="px-6 py-4 text-right text-sm font-medium">
                        <a
                          onClick={() =>
                            navigate(
                              `/admin/asignaturas/edit/${asignatura._id}`
                            )
                          }
            ...
            <div className="flex justify-between items-center mt-4">
                <div>
                    <p className="text-sm text-text-secondary">
                        Mostrando {indexOfFirstItem + 1} a {Math.min(indexOfLastItem, asignaturas.length)} de {asignaturas.length} asignaturas
                    </p>
                </div>
                <div className="flex items-center">
                    <CustomButton
