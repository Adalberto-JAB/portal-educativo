  ...
  const fetchCourseData = useCallback(async () => {
    try {
      ...
      const isCourseAuthor =
        user && fetchedCourse.author && fetchedCourse.author._id === user.id;
      ...
      const nivelValue =
        typeof fetchedCourse.nivel === "object" && fetchedCourse.nivel !== null
          ? fetchedCourse.nivel._id
          : fetchedCourse.nivel;
      ...
      if (fetchedCourse.cover) {
        setCurrentCoverUrl(
          `${import.meta.env.VITE_BACKEND_API_URL}/api/covers/${
            fetchedCourse.cover._id
          }`
        );
      ...
      if (isAdmin) {
        ...
        const availableAuthors = fetchedUsers.filter(
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            ...
            toast.error(
              err.response?.data?.message || "Error al eliminar la lección."
            );
          }
        }
      });
          ...
          >
            ...
            <div>
              ...
              >
                ...
                {niveles.map((nivel) => (
                  <option key={nivel._id} value={nivel._id}>
                    {nivel.name}
                  </option>
                ))}
              </select>
            ...
            <div>
              <label
                ...
                htmlFor="coverImage"
              >
                Imagen de Portada (PNG/JPG opcional):
              </label>
              <input
                type="file"
                id="coverImage"
              ...
              <>
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id="isGuestViewable"
                    {...register("isGuestViewable")}
                    className="form-checkbox h-5 w-5 text-accent-primary rounded"
                  />
                  <label
                    className="text-text-primary text-sm"
                    htmlFor="isGuestViewable"
                  >
          ...
          <div className="mt-8 border-t border-border-color pt-6">
            ...
            {canManageLessons && (
              <div className="mb-4">
                <CustomButton
                  variant="primary"
                  onClick={() =>
                    navigate(`/admin/courses/${id}/lessons/create`)
                  }
                >
            ...
            {lessons.length === 0 ? (
              <p className="text-text-secondary">
                No hay lecciones para este curso aún.
              </p>
            ) : (
              <ul className="space-y-3">
                {lessons.map((lesson) => (
                  <li
                    key={lesson._id}
                    className="bg-bg-primary p-4 rounded-lg shadow-sm border border-border-color flex items-center justify-between"
                  >
                      ...
                      {canManageLessons && (
                        <>
                          <button
                            onClick={() =>
                              navigate(
                                `/admin/courses/${id}/lessons/edit/${lesson._id}`
                              )
                            }
