...
const DocumentationManagementPage = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          toast.error(
            error.response?.data?.message || `Error al ${action} el documento.`
          );
        }
      }
    });
  ...
  const handleToggleGuestViewable = async (doc) => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Error al actualizar la visibilidad."
      );
    }
  };
    ...
    <div className="mt-28 p-8 min-h-[calc(100vh-80px)] bg-bg-primary text-text-primary">
      <div className="max-w-[1024px] mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <CustomButton onClick={() => navigate("/documentation/create")}>
            Añadir Documento
          </CustomButton>
        </div>
          ...
          <table className="w-full table-auto divide-y divide-border-color">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-6 py-3 text-right text-xs font-medium text-text-secondary uppercase tracking-wider min-w-54">
                  Acciones
                </th>
              </tr>
            </thead>
            <tbody className="bg-bg-secondary divide-y divide-border-color">
              {docs.map((doc) => (
                <tr key={doc._id}>
                  <td className="px-6 py-4 text-text-primary break-words font-semibold">
                    {doc.title}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right text-sm font-medium">
                    ...
                    <button
                      onClick={() => handleToggleGuestViewable(doc)}
                      className={`mr-4 ${
                        doc.isGuestViewable
                          ? "text-blue-600 hover:text-blue-900"
                          : "text-gray-400 hover:text-gray-600"
                      }`}
                      title="Visible para Invitados"
                    >
                      {doc.isGuestViewable ? (
                        <FaEye size={18} />
                      ) : (
